...
def test_kuaishou_selects_requested_photo_and_exports_public_account() -> None:
    recommendation = {
        "caption": "推荐视频文案",
        "userName": "推荐作者",
        "userId": 999000111,
    ...
    data = _run(
        KuaishouExtractor().extract(
            FakePage({}),
            document,
            _definition("kuaishou"),
        )
    )
    ...
    assert data.published_at is not None
    assert data.published_at.strftime("%Y-%m-%d %H:%M:%S") == "2026-07-28 23:19:35"


def test_kuaishou_rejects_recommendations_when_requested_photo_is_missing() -> None:
    document = RenderedDocument(
        url=(
