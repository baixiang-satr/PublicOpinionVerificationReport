...
7. 不实现或引入代理池、反检测、验证码处理、权限绕过或未经授权的浏览器控制。
8. 异步任务、浏览器、Excel COM 和文件操作必须支持取消、超时、错误分类及资源释放。
9. 每次行为改动必须补充或更新对应测试；不得依赖真实外网、真实 Cookie 或源模板写入。
10. 完成前必须运行与改动匹配的检查，并同步更新受影响的文档、模块说明和任务状态。
